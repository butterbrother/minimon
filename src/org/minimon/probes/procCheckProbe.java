...
public class procCheckProbe
    ...
    public boolean iteration() {
        ...
        try {
            // Исполняем
            ps.execute();
            BufferedReader errors = ps.getStderr();
            BufferedReader result = ps.getStdout();
            ps.waitFor();
            ...
            while ((buffer = result.readLine()) != null) {
                // Если нашли
                if (buffer.toLowerCase().contains(procCheck)) {
                    errors.close();
                    result.close();
                    log.debug("String - " + buffer + ", process name - " + procCheck + ", found");
                    return true;
                }
            }
