...
public class httpProbe
	...
	public boolean prepare(logger log, boolean debugState) {
		...
		log.debug("Base parameters parsed");
	...
	public boolean iteration() {
		...
		try {
			log.debug("Prepare wget execute");
			wget.execute();
			log.debug("Wget executed");
			StreamGrabber results = new StreamGrabber(wget.getStdout(), "", log.getModuleSubLogger("StdOut grabber"));
			StreamGrabber errors = new StreamGrabber(wget.getStderr(), "<stderr>", log.getModuleSubLogger("StdErr grabber"));
			wget.waitFor();

			// Получаем stdErr
			lastError += errors.getResults();
